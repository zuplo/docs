...
In order to authorize your Zuplo API to upload to the GCP bucket, you will need
to configure the
[Upstream GCP Service Auth Policy](/docs/policies/upstream-gcp-service-auth-inbound).
...
that authorizes your Zuplo API to call downstream Cloud Run services.

Each auth policy will cache the JWT tokens for an hour by default so having
multiple policies will have virtually no impact on your API's latency.
