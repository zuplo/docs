...
function OptionType({ value }: { value: JSONSchema7 }) {
  ...
  if (value.type === "array") {
    ...
    typeString = value.oneOf
      .map((o: JSONSchema7Definition) => {
        const obj = o as JSONSchema7;
        const items = obj.items as JSONSchema7;

        if (items && items.type) {
          return `${items.type.toString()}[]`;
        }
...
function getPolicyFilePaths(policyDir: string) {
  return {
...
async function getExampleHtml(
  ...
  if (
    properties &&
    properties.options &&
    Object.keys(properties.options).length === 0
  ) {
    console.warn(
      chalk.yellow(
