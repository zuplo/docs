...
clients calling your API using certificates issued by a Certificate Authority
(CA) that you control. Both the client and the gateway authenticate each other
during the TLS handshake, so only clients holding a certificate signed by your
CA can reach your API.

This is useful when you need strong, cryptographic authentication of partners,
devices, or services — for example IoT fleets, B2B integrations, or zero-trust
internal APIs — without distributing API keys or OAuth credentials.
...
- `certIssuerDN` (optional): The fully qualified issuer distinguished name that
  the client certificate must be signed by. Set this when you have multiple CAs
  on your account and want a given route to accept certificates from only one of
  them. Setting this by default is recommended.
...
2. Reissue client certificates from the new CA and distribute them to your
   clients.
3. If your route uses `certIssuerDN`, update it to accept the new issuer
   (temporarily allow either issuer at the application level, or stage the
   change behind `allowUnauthenticatedRequests: true` during cutover).
4. Once all clients have moved to the new CA, delete the old CA with
   `zuplo ca-certificate delete`.
...
gateway domains. If you use a custom domain and your clients aren't being
verified against it, contact [support@zuplo.com](mailto:support@zuplo.com).
