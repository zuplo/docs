...
The following environment variables are provided by default and can be used in
code or configuration.

- `ZUPLO_PROJECT_ID` - The unique internal identifier of your project. This is a
  `string` that starts with `prj_`. For example, `prj_SPP0LmkilwFKlAUCujCGUutV`.
- `ZUPLO_ENVIRONMENT_TYPE` - This indicates the environment type where the
  deployment is running. Values can be `WORKING_COPY`, `PREVIEW`, or
  `PRODUCTION`.
