...
jobs:
  run-zup-test:
    ...
    steps:
      ...
      # Note that you aren't required to use tee. We're using it in this example
  ...
  branches:
    ...
    main:
      ...
      - step:
          ...
          # set -o pipefail
          # This way if the deploy fails, we fail before piping to tee.
          # Note that you aren't required to use tee. We're using it in this example so that the output is available to the terminal and written to the file.
          script:
            - set -o pipefail
