...
captured. We are working towards full coverage.
...
- **Action**: The specific operation performed (e.g., `project.create`,
...
- **ActingAs**: Information about impersonated user, if applicable
...
- **Source**: The source system or API that handled the request (e.g., 'api',
  'gateway')
...
:::note Date Range Limitation The date range between `startDate` and `endDate`
cannot exceed 30 days. The start date must be before the end date. :::
