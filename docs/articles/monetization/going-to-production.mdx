...
- The **pre-production checklist** — items to verify before enabling real
  charges
...
1. Click **Configure** on the Stripe card.
...
![Zuplo Portal Monetization Service with the Prod environment selected and the Payment Provider tab open, showing the Stripe Connected card, Stripe API Key field, Billing Profiles, and Payment Overage configuration](../../../public/media/monetization-going-to-production/configure-stripe-production.png)
