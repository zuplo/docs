...
By default, Zuplo sends metrics to Datadog in `seriesMode` for backwards
compatibility. You can switch to `distributionMode` if you want Datadog
distribution ingestion and percentile/count analytics across edge replicas.
...
| `distributionMode` | `/api/v1/distribution_points` | `zuplo.request.latency.v2`, `zuplo.request.content_length.v2`, `zuplo.response.content_length.v2` | Accurate request populations and percentile queries (`p95`, `p99`, etc.) |

When you use `distributionMode`, all three metrics are sent as Datadog
distributions and use the `.v2` suffix.

### Series mode (default / backwards compatible)
