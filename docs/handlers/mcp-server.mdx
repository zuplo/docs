...
[MCP Quickstart](../articles/mcp-quickstart.mdx).
...
Configure MCP tools using the `files` configuration. Specify OpenAPI files and
the exact operation IDs you want to expose as MCP tools:
            ...
            "files": [
              {
                ...
                "operationIds": ["getCurrentWeather", "getForecast"]
              },
              {
                "path": "./config/todos.oas.json",
                "operationIds": ["getTodos", "createTodo", "updateTodo"]
              }
            ]
          }
        }
...
- `path`: Path to an OpenAPI JSON spec file (relative to the project root)
- `operationIds`: Array of specific operation IDs to include as MCP tools from
  the targeted file

This approach provides explicit control over exactly which API operations become
MCP tools and supports OpenAPI extensions for enhanced configuration (see
OpenAPI Configuration section below).

#### Deprecated Configurations

:::caution

Both `openApiFilePaths` and `openApiTools` configuration options are
**deprecated** and will be removed in a future version. Please migrate to the
`files` configuration option described above.

:::

<details>
<summary>Option 1: Transform Entire OpenAPI Files (Deprecated)</summary>
...
<details>
<summary>Option 2: Transform Individual Routes (Deprecated)</summary>
...
**After (using `files` and individual operation IDs):**
    ...
    "files": [
      {
        ...
        "operationIds": ["operation1", "operation2", "operation3"]
      }
    ]
  }
}
...
using the `prompts` array in your MCP server options:
      ...
      "x-zuplo-route": {
        "handler": {
          ...
          "options": {
            ...
            "prompts": [
              {
                "path": "./config/prompts.oas.json",
                "operationIds": ["greeting_generator", "content_template"]
              }
            ]
          }
        }
...
Routes configured as MCP prompts may include the `x-zuplo-mcp-prompt` extension
in their OpenAPI definition in order to provide additional MCP metadata. These
routes should return structured message arrays from their handlers. See
...
Like tools, configure resources using the `resources` array in your MCP server
options with operation IDs of routes in your gateway:
...
Routes configured as MCP resources must use the GET method and may include the
`x-zuplo-mcp-resource` extension in their OpenAPI definition to provide
...
- **Tool names**: Uses the route's `operationId` if available, otherwise falls
  back to a generated `METHOD_ROUTE` format (for example, `GET_todos`)
...
### `x-zuplo-mcp-tool` configuration

You can customize individual tools using the `x-zuplo-mcp-tool` OpenAPI
extension: this is especially useful if changing the `operationId` or
`description` on a route isn't possible but you want to provide better context
...
**x-zuplo-mcp-tool options:**

- `name` (optional): Override the tool name shown to AI systems (defaults to
  `operationId`)
- `description` (optional): Override the tool description for AI consumption
  (falls back to `description` and then `summary` if not set)
- `enabled` (default: `true`): Whether this operation should be available as an
  MCP tool in a configured MCP server: useful for debugging or fine grained
  control.
