...
Let's proxy the sample e-commerce API at `https://ecommerce-api.zuplo.io` with
...
Change the **version** to be `v1` and the **path** of the new route to be `/products/(.*)` - this uses a
...
path to be `https://ecommerce-api.zuplo.io/v1/products/${params[0]}`. A parameter
